my_data <- read.csv('https://raw.githubusercontent.com/sakunisgithub/data_sets/master/doe_prac_q13_data.csv', stringsAsFactors = TRUE)
dim(my_data)
names(my_data)
str(my_data)
View(my_data)
...
proper_trt_comb_order <- c("(1)", "n", "p", "np", "k", "nk", "pk", "npk")

df_treatment_combination <- order_trt_comb_col(dataframe = df_treatment_combination,
                                               column_number = 1,
                                               proper_order = proper_trt_comb_order)
...
yates_operations$sum_squares <- round(yates_operations$sum_squares, digits = 4)
...
round(yates_operations$sum_squares[2:7] / MSE, digits = 4)
