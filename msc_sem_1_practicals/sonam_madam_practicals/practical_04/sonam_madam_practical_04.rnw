...
Observed $\chi^2 = \Sexpr{observed_chi_sq} < \chi^2_{0.05, 5} = \Sexpr{qchisq(0.05, 5, lower.tail = FALSE)}$. So we fail to reject the null hypothesis of goodness of fit test and conclude that there is not enough evidence to claim that the given data is not from a Binomial population.
