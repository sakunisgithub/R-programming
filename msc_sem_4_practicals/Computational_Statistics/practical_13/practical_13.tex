...
To generate random numbers from $\beta_1$ distribution, we recall
...
If $x_1, x_2, \ldots, x_n$ is a sample of size $n$ from $X$, Monte Carlo estimate of $\theta = E[X]$ is $$\widehat{\theta} = \dfrac{1}{n} \sum \limits_{i = 1}^{n} x_i.$$

Estimate of $\theta$ by using antithetic variable will be

$$\widehat{\theta} = \dfrac{1}{2n} \sum \limits_{i = 1}^{n} [X_i + X_i']$$
...
% Date and time: Mon, Apr 06, 2026 - 17:31:14
...
\hlkwa{for} \hldef{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{length}\hldef{(sample.sizes)) \{}
  ...
  \hldef{x} \hlkwb{<-} \hlkwd{my.rbeta}\hldef{(size,} \hlnum{2}\hldef{,} \hlnum{3}\hldef{, u)}
  \hldef{x_dash} \hlkwb{<-} \hlkwd{my.rbeta}\hldef{(size,} \hlnum{2}\hldef{,} \hlnum{3}\hldef{,} \hlnum{1}\hlopt{-}\hldef{u)}
...
% Table created by stargazer v.5.2.3 by Marek Hlavac, Social Policy Institute. E-mail: marek.hlavac at gmail.com
% Date and time: Mon, Apr 06, 2026 - 17:31:16
\begin{table}[!htbp] \centering 
...
% Table created by stargazer v.5.2.3 by Marek Hlavac, Social Policy Institute. E-mail: marek.hlavac at gmail.com
% Date and time: Mon, Apr 06, 2026 - 17:31:19
\begin{table}[!htbp] \centering 
