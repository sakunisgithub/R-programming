...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{q} \hlkwb{<-} \hlkwa{function}\hldef{(}\hlkwc{x}\hldef{)} \hlnum{1}
\end{alltt}
\end{kframe}
\end{knitrout}
...
Thus we have a sample of size $986$ from $p(x)$. \\[0.3em]

The \textbf{acceptance rate} was $0.493$ whereas the \textbf{theoretical acceptance rate} is $0.4739336$.
...
\hlkwd{data.frame}\hldef{(}\hlkwc{x} \hldef{=} \hlkwd{c}\hldef{(}\hlnum{0}\hldef{,} \hlnum{1}\hldef{))} \hlopt{%>%}
  ...
  \hlkwd{geom_point}\hldef{(}\hlkwc{data} \hldef{=} \hlkwd{data.frame}\hldef{(}\hlkwc{x} \hldef{= x,} \hlkwc{y} \hldef{= y),} \hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= x,} \hlkwc{y} \hldef{= y),} \hlkwc{size} \hldef{=} \hlnum{2}\hldef{)} \hlopt{+}
  \hlkwd{geom_point}\hldef{(}\hlkwc{data} \hldef{=} \hlkwd{data.frame}\hldef{(}\hlkwc{x} \hldef{= x[good],} \hlkwc{y} \hldef{= y[good]),}
             \hlkwd{aes}\hldef{(}\hlkwc{x} \hldef{= x,} \hlkwc{y} \hldef{= y),} \hlkwc{size} \hldef{=} \hlnum{2}\hldef{,} \hlkwc{col} \hldef{=} \hlsng{"green"}\hldef{)}
\end{alltt}
\end{kframe}
