...
h_t_hat <- number_of_patients_dying_per_unit_time_in_the_interval / ( survival_data$number_alive_at_the_beginning_of_the_interval * interval_width)

h_t_hat[length(h_t_hat)] = NA

analysis_table <- data.frame(survival_data, t = 0:9, S_t_hat = round(S_t_hat, digits = 3), f_t_hat = round(f_t_hat, digits = 3), h_t_hat = round(h_t_hat, digits = 3))
