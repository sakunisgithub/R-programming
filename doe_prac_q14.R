my_data <- read.csv('https://raw.githubusercontent.com/sakunisgithub/data_sets/master/doe_prac_q14_data.csv', stringsAsFactors = TRUE)
dim(my_data)
names(my_data)
str(my_data)
View(my_data)
...
proper_treatment_combination_order <- c("(1)", "n", "k", "nk", "p", "np", "kp", "nkp")

df_treatment_combinations <- order_trt_comb_col(dataframe = df_treatment_combinations,
                                                column_number = 1,
                                                proper_order = proper_treatment_combination_order)
...
MSblocks <- round(SSblocks / 7, digits = 3)
MSblocks

MStreatments <- round(SStreatments / 6, digits = 3)
MStreatments

MSE <- round(SSE / 18, digits = 3)
MSE
...
round(yates_operations$sum_squares[2:7] / MSE, digits = 3)
